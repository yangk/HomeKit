...
int tcp_client_func(struct iperf_data_t iperf_data)
{
    ...
    if(iperf_data.total_size == 0){
        ...
        while ( ((end_time - start_time) <= (configTICK_RATE_HZ * iperf_data.time)) && (!g_tcp_terminate) ) {
            if( send(client_fd, buffer, iperf_data.buf_size,0) <= 0){
            ...
            else if((end_time - report_start_time) > (configTICK_RATE_HZ * 2)){
                report_start_time = end_time;
                report_size = 0;
            }
        }
        ...
        report_start_time = xTaskGetTickCount();
        end_time = report_start_time;
...
int tcp_server_func(struct iperf_data_t iperf_data)
{
    ...
    int                  recv_size=0;
    uint64_t             total_size=0,report_size=0;
    uint32_t             report_start_time, report_end_time;

    buffer = pvPortMalloc(iperf_data.buf_size);
    if(NULL == buffer){
        printf("\n\r[ERROR] %s: Allocate server buffer failed",__func__);
        return -1;
    }
    ...
    printf("\n\r%s: Create socket fd = %d", __func__,server_fd);
...
Restart:
    ...
    while (!g_tcp_terminate) {
        recv_size = recv(client_fd, buffer, iperf_data.buf_size, 0);  //MSG_DONTWAIT   MSG_WAITALL
        if( recv_size < 0){
            printf("\n\r[ERROR] %s: Receive data failed",__func__);
            goto Exit1;
        }
        else if(recv_size == 0){
            printf("\n\r%s: Receive %d KBytes packets",__func__,(uint32_t) (total_size/KB));
            total_size=0;
            close(client_fd);
            goto Restart;
        }
...
int udp_server_func(struct iperf_data_t iperf_data)
{
    ...
    printf("\n\r%s: Create socket fd = %d, port = %d", __func__,server_fd,iperf_data.port);
    ...
    while (!g_udp_terminate) {
        recv_size = recvfrom(server_fd,buffer,iperf_data.buf_size,0,(struct sockaddr *) &client_addr,&addrlen);
        if( recv_size < 0){
            printf("\n\r[ERROR] %s: Receive data failed",__func__);
            goto Exit1;
        }
    ...
    printf("\n\r%s: Totally receive %d KBytes",__func__, (uint32_t) (total_size/KB));
Exit1:
    // close the listening socket
    close(server_fd);
